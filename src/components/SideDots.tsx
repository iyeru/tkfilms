 ...
 */
export function SideDots({ nav, active }: { nav: NavItem[]; active: SectionId }) {
  return (
    <div
      aria-hidden
      className="fixed top-1/2 right-[34px] z-99 hidden -translate-y-1/2 flex-col gap-3.5 wide:flex"
    >
      {nav.map((item) => (
        <a
          key={item.id}
...
export function BackToTop({ visible }: { visible: boolean }) {
  return (
    <button
      ...
      className={cn(
        'fixed right-5 bottom-5 z-98 h-12 w-12 border-0 text-base text-white md:right-10 md:bottom-10 md:h-[50px] md:w-[50px]',
        'bg-[linear-gradient(to_top,var(--color-accent)_50%,#333_50%)] bg-[length:100%_200%] bg-[position:0_0]',
